...
class TestSequences:
    ...
    def test_brownian_bridge(
        ...
        if multichannel:
            ...
            bridge = geo.sample_brownian_bridge(
    ...
    def test_straight_line(self, einstein_img, curie_img, n_steps, multichannel):
        if multichannel:
            einstein_img = einstein_img.repeat(1, 3, 1, 1)
            curie_img = curie_img.repeat(1, 3, 1, 1)
            line = geo.make_straight_line(einstein_img, curie_img, n_steps)
            assert line.shape == (
                n_steps + 1,
                *einstein_img.shape[1:],
...
class TestGeodesic:
    ...
    @pytest.mark.parametrize("model", ["frontend.OnOff.nograd"], indirect=True)
    @pytest.mark.parametrize(
        "fail", [False, "img_a", "img_b", "model", "n_steps", "range_penalty"]
    )
    def test_save_load(self, einstein_small_seq, model, fail, tmp_path):
        ...
        range_penalty = 0
        moog = geo.Geodesic(
            img_a, img_b, model, n_steps, range_penalty_lambda=range_penalty
        )
        ...
        if fail:
            if fail == "img_a":
                ...
                expectation = pytest.raises(
                    ValueError, match="Saved and initialized image_a are different"
                )
            elif fail == "img_b":
                img_b = torch.rand_like(img_b)
                expectation = pytest.raises(
                    ValueError, match="Saved and initialized image_b are different"
                )
            elif fail == "model":
                model = po.simul.Gaussian(30).to(DEVICE)
                po.tools.remove_grad(model)
                expectation = pytest.raises(
                    ValueError, match="objective_function on pixelfade of saved"
                )
            elif fail == "n_steps":
                n_steps = 5
                expectation = pytest.raises(
                    ValueError, match="Saved and initialized n_steps are different"
                )
            ...
            moog_copy = geo.Geodesic(
                img_a, img_b, model, n_steps, range_penalty_lambda=range_penalty
            )
            with expectation:
                moog_copy.load(
                    op.join(tmp_path, "test_geodesic_save_load.pt"), map_location=DEVICE
                )
        else:
            moog_copy = geo.Geodesic(
                img_a, img_b, model, n_steps, range_penalty_lambda=range_penalty
    ...
    def test_initialization(self, einstein_small_seq, model, init):
        expectation = does_not_raise()
        if init == "succeed":
            init = geo.make_straight_line(
                einstein_small_seq[:1], einstein_small_seq[-1:], 5
            )
        elif init == "fail-start":
            init = geo.make_straight_line(
                einstein_small_seq[1:2], einstein_small_seq[-1:], 5
            )
            expectation = pytest.raises(
                ValueError, match="First frame of initial_sequence"
            )
        elif init == "fail-stop":
            init = geo.make_straight_line(
                einstein_small_seq[:1], einstein_small_seq[-2:-1], 5
            )
            expectation = pytest.raises(
                ValueError, match="Last frame of initial_sequence"
            )
        elif init == "fail-len":
            init = geo.make_straight_line(
                einstein_small_seq[:1], einstein_small_seq[-1:], 6
            )
            expectation = pytest.raises(
            ...
            expectation = pytest.raises(
                ValueError, match="initial_sequence, image_a, and image_b"
            )
        elif init == "fail-dim":
            init = geo.make_straight_line(
                einstein_small_seq[:1], einstein_small_seq[-1:], 5
